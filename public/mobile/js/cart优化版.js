...
Letao.prototype = {
                ...
                up: {
                    ...
                    callback: function() {
                        ...
                        setTimeout(function() {
                            ...
                            // 4. 调用获取购物车数据的公共方法
                            that.queryCartData(function(data) {
                                // 5. 数据返回有点问题 返回是一个[]空数组 而不是一个对象里面的data值为空  判断当前数据是否还有值 判断data.data.length 是否大于0
    ...
    editCart: function() {
        ...
        $('#cartList').on('tap', '.btn-edit', function() {
            // 2. 获取当前点击的编辑按钮的父元素的父元素 li
            var li = this.parentNode.parentNode;
            // 2. 准备一个编辑的尺码和数量的模板
            // 3. 准备模需要的数据  通过自定义属性获取的
            //     1.商品的所有尺码 
            //     2.当前选择的尺码 
            //     3.所有商品的数量 
            //     4.当前选择的数量   
            ...
            // 6. 把生成的html字符串去掉里面的回车和换行如果不去掉会变成br标签
            html = html.replace(/[\r\n]/g, "");
            // 7. 弹出一个确认框  把准备好的模板放到确认框的内容
